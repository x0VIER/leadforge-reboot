...
$findings = foreach ($row in $rows) {
    ...
    if ($row.public_email -and -not (Test-CommonMailboxDomain $row.public_email) -and $websiteHost -and $emailHost -and -not $emailHost.EndsWith($websiteHost) -and -not $websiteHost.EndsWith($emailHost)) { $flags += 'email_domain_mismatch' }
