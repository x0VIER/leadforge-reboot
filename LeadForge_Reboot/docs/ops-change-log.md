...
- Timing fix: `config/source-lanes.json` now uses `overpassTimeoutMs = 15000`, `maxLaneNicheChecksPerRun = 9`, and `querySampleLimit = 6`. This gives slower public Overpass queries enough time to return while reducing checks per sprint so the guarded collector stays inside the 180-second runtime cap.
