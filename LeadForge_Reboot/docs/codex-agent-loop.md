...
2. Hermes refreshes `agent_shared/status/PENDING_ENRICHMENT_QUEUE.json`, `agent_shared/status/OWNER_ENRICHMENT_BACKLOG_FL.json`, `agent_shared/status/MASTER_CONTAMINATION_AUDIT_FL.json`, and `agent_shared/status/OPS_SNAPSHOT.json`, then runs `scripts/get-collector-guard-status.ps1` before starting new sourcing.
